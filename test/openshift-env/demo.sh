...
praxis_uid=$(jq -r '.items[0].metadata.uid//empty' "$RUN_ROOT/praxis-pods.json" 2>/dev/null || true)
praxis_restart=$(jq -r '([.items[0].status.containerStatuses[]?.restartCount]|add)//empty' "$RUN_ROOT/praxis-pods.json" 2>/dev/null || true)
...
praxis_image=$(jq -r '.items[0].spec.containers[]?.image//empty' "$RUN_ROOT/praxis-pods.json" 2>/dev/null|head -1 || true)

printf '%s\n\n' 'OpenShift External Models -> Praxis narrative demonstration'
printf '%s\n\n' 'A platform engineer opts one MaaS tenant into standalone Praxis processing, authenticates through the Gateway, and changes the selected test endpoint while the tenant data plane remains running. This is a single-tenant integration demonstration using real reconciliation and request evidence.'
printf '%s\n' 'ARCHITECTURE / REQUEST PATH' 'client -> Gateway/Envoy -> Kuadrant/Authorino -> Praxis ExtProc -> tenant-local Praxis -> provider fixture' ''
...
s=$(status_for 'controller deployment Ready|AITenant Ready|ExternalProvider Ready|ExternalModel Ready|standalone Praxis Ready')
block 1 'Deployment readiness' 'Establishes the test environment before request testing; readiness alone does not prove routing.' 'The authoritative suite waited for controller, MaaS tenant, provider/model fixtures, and tenant-local Praxis readiness.' "$(detail 'controller deployment Ready|AITenant Ready|ExternalProvider Ready|ExternalModel Ready|standalone Praxis Ready')" "$s" 'The recorded readiness assertions are the foundation for the later request tests. Gateway, Authorino, and ExtProc behavior is not inferred from pod readiness.'

s=$(status_for '^AITenant Ready$'); [[ -n "$tenant_ns" && -n "$gateway" && -s "$RUN_ROOT/tenant-resources.json" ]] || s='NOT DEMONSTRATED'
block 2 'Tenant Praxis selection' 'Shows explicit Praxis opt-in, resolved tenant namespace, tenant-local workload, routing state, and projected credential reference.' 'The suite inspected the AITenant and tenant resources after reconciliation.' "opt-in=praxis; resolved namespace=$(safe "${tenant_ns:-missing}"); gateway=$(safe "${gateway_ns:-missing}/${gateway:-missing}"); tenant resources=$(file_state tenant-resources.json)" "$s" 'The controller owns tenant-local Praxis resources and routing projection while MaaS supplies tenant selection. Missing fields cannot be treated as a pass.'
...
block 3.5 'External HTTPS transport' 'Confirms the mounted Praxis configuration preserves provider authority, TLS, hostname-only SNI, and the explicit dial port.' 'The suite inspected the live tenant Praxis configuration for both declared Chat Completions providers.' "transport evidence=$(file_state praxis-transport-config.txt); API contract=openai-chat /v1/chat/completions" "$s" 'This proves the rendered request transport contract. It does not add or imply support for /v1/responses.'
...
s=$(status_for '^First test endpoint$'); [[ -n "$praxis_uid" && -n "$praxis_restart" ]] || s='NOT DEMONSTRATED'
block 5 'Provider A request' 'Proves the authenticated Gateway -> Authorino -> ExtProc -> Praxis -> provider path.' 'The suite sent an authenticated request after Provider A overlay convergence and checked trusted attribution.' "$(detail '^First test endpoint$'); Praxis UID=$(safe "${praxis_uid:-missing}"); restart count=$(safe "${praxis_restart:-missing}")" "$s" 'HTTP 200 with Provider A attribution proves the complete request path reached the selected fixture. The Praxis identity is recorded as the process-stability baseline.'
...
s=$(status_for '^Second test endpoint$|^Praxis identity stable$'); [[ -n "$digest" && -n "$mounted_digest" ]] || s='NOT DEMONSTRATED'
block 7 'Provider A to Provider B switch' 'Demonstrates a controlled routing update while Praxis remains running.' 'The suite changed the declared provider selection, waited for projection, checked mounted state, and sent a new request.' "$(detail '^Second test endpoint$|^Praxis identity stable$'); old/new generation and digest=not separately serialized; declared/recomputed/mounted evidence=$(file_state config-overlay.json)/$(file_state mounted-overlay.json); current digest=$(safe "${digest:-missing}")" "$s" 'Provider B HTTP 200 and attribution prove the selected update. This is not automatic failover or load balancing. Exact old/new revision values were not separately serialized by this retained e2e result.'

block 8 'Semantic no-op' 'Checks that unchanged effective routing does not publish a new configuration or restart Praxis.' 'The suite reapplied unchanged model content and compared generation, resource version, and digest.' "$(detail '^Semantic no-op$')" "$(status_for '^Semantic no-op$')" 'The result is based on recorded before/after identity and digest comparisons, not a sleep-based assumption.'
...
block 10 'Provider credential enforcement' 'Proves the backend rejects absent or incorrect credentials and that caller credentials cannot replace the projected provider credential.' 'The authenticated Gateway request carries the MaaS API key in Authorization while the backend accepts only its distinct provider key. A separate request supplies a conflicting x-api-key.' "$(detail "$credential_re"); credential values=withheld" "$(status_for "$credential_re")" 'Direct HTTP 401 responses prove backend enforcement. The attributed HTTP 200 responses prove Praxis replaced the caller Authorization value and ignored the conflicting x-api-key. Duplicate Authorization header ordering is out of scope. Credential rotation, OAuth2, and SigV4 are not covered.'

block 11 'Tenant security' 'Checks Secret API isolation for the Praxis ServiceAccount and evidence hygiene.' 'The suite performed the ServiceAccount authorization check and scanned finalized output for credential-shaped patterns.' "$(detail '^Praxis Secret API denied$|^Credential leak scan$')" "$(status_for '^Praxis Secret API denied$|^Credential leak scan$')" 'The result proves Kubernetes API isolation and evidence hygiene for this run; Secret bytes and the temporary API key are not displayed.'
...
names=('Deployment readiness' 'Tenant Praxis selection' 'Route acceptance' 'External HTTPS transport' 'TLS and authentication' 'Provider A request' 'Unknown model' 'Provider switch' 'Praxis remained running' 'Semantic no-op' 'Last-known-good behavior' 'Credential enforcement' 'Tenant security' 'Provider A reset' 'Credential revocation')
values=("$(status_for 'controller deployment Ready|AITenant Ready|ExternalProvider Ready|ExternalModel Ready|standalone Praxis Ready')" "$( [[ -n "$tenant_ns" && -n "$gateway" && -s "$RUN_ROOT/tenant-resources.json" ]] && printf PASS || printf 'NOT DEMONSTRATED' )" "$(status_for '^HTTPRoute Accepted$|^HTTPRoute ResolvedRefs$')" "$(status_for '^External HTTPS transport$')" "$(status_for '^Unauthenticated request$|^API-key creation$')" "$(status_for '^First test endpoint$')" "$(status_for '^Unknown model$')" "$(status_for '^Second test endpoint$|^Praxis identity stable$')" "$(status_for '^Praxis identity stable$')" "$(status_for '^Semantic no-op$')" "$(status_for '^Invalid overlay LKG$')" "$(status_for "$credential_re")" "$(status_for '^Praxis Secret API denied$|^Credential leak scan$')" "$(status_for '^Provider A reset request$')" "$(status_for '^API-key revocation$')")
printf '\nVALIDATION SUMMARY\n\n%-40s %s\n%-40s %s\n' 'Test' 'Result' '----------------------------------------' '----------------'
...
printf 'Praxis UID: %s\nPraxis restart count: %s\nOverlay digest: %s\nMounted digest: %s\n' "$(safe "${praxis_uid:-not recorded}")" "$(safe "${praxis_restart:-not recorded}")" "$(safe "${digest:-not recorded}")" "$(safe "${mounted_digest:-not recorded}")"
printf 'Image digests:\n  controller: %s\n  Praxis: %s\n  ExtProc: %s\n  MaaS: not recorded in this e2e result\n  provider fixtures: not recorded in this e2e result\n' "$(safe "${controller_image:-not recorded}")" "$(safe "${praxis_image:-not recorded}")" "$(safe "${extproc_image:-not recorded}")"
