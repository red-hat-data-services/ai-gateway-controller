# ADR: ExternalModel/ExternalProvider → Praxis — two-plane authority and the routing-overlay contract

|                |            |
| -------------- | ---------- |
| Date           | 2026-09-08 |
| Scope          | RHOAI AI gateway — `ai-gateway-controller` (control plane) + `praxis-extproc` (data plane), 3.6 external-model migration |
| Status         | Proposed (accepts at the control-plane/data-plane/MaaS walkthrough; "Proposed" per the org ADR convention until agreed) |
| Authors        | Yossi Ovadia(@yossiovadia) |
| Supersedes     | N/A |
| Superseded by: | N/A |
| Tickets        | RHOAI 3.6 external-model migration (epic link TBD) |
...
Format: [opendatahub-io/architecture-decision-records](https://github.com/opendatahub-io/architecture-decision-records) house template (`ODH-ADR-0000`). On acceptance this record can be promoted to the org ADR repo under the `ODH-ADR-<component>-NNNN` naming.
